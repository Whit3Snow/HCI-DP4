...
var countid4 = 0;
var countid5 = 0;
class OpenDiary extends React.Component{
...
function read(){
    db.collection("Groups").doc("HELLO BADMINTON").collection("Diary").doc("w34xowedr0uUsU1qwMjq").get()
    .then(function(doc){
        if (doc.exists){
            for (var i = 0; i < doc.data().icon1_input.length; i++){
                icon1(doc.data().icon1_input[i], doc.data().icon1_text, doc.data().icon1_url, parseInt(doc.data().icon1_locate[i]["x"])-150, doc.data().icon2_locate[i]["y"]);
            }
            ...
            for (var i = 0; i < doc.data().icon4_input.length; i++){
                icon4(doc.data().icon4_input[i], doc.data().icon4_locate[i]["x"], doc.data().icon4_locate[i]["y"])
            }
            for (var i = 0; i < doc.data().icon5_input.length; i++){
                icon5(doc.data().icon5_input[i], doc.data().icon5_locate[i]["x"], doc.data().icon5_locate[i]["y"])
            }
